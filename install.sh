...
read -p "Which setup are you going to do? (kernel/wifi/bluetooth): " setup_choice

case $setup_choice in
    kernel)
        echo "Setting up for Kernel tests..."
        git clone --recurse-submodules https://github.com/linux-test-project/ltp.git "${LTP_DIR}"
        ;;
    wifi)
        echo "Setting up for Wi-Fi tests..."
        # Additional commands for Wi-Fi setup
        ;;
    bluetooth)
        echo "Setting up for Bluetooth tests..."
        # Additional commands for Bluetooth setup
        ;;
    *)
        echo "Invalid choice. Please select kernel, wifi, or bluetooth."
        ;;
esac
